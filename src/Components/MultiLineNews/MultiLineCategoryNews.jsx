...
const MultiLineCategoryNews = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const API_URL = `https://linesnews.onrender.com/api/multi-line-newss?filters[category][$eq]=${category}&pagination[page]=${page}&pagination[pageSize]=10&sort=publishedAt:desc`;
    axios.get(API_URL)
      .then((response) => {
  ...
  return (
    <>
      <MultiLineCategory setCategory={setCategory} />
      <Header />
...
export default MultiLineCategoryNews;
